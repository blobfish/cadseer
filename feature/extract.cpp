...
void Extract::updateModel(const UpdatePayload &payloadIn)
{
  ...
  try
  {
    ...
    //TODO add normal picks.
    if (faces.empty())
      throw std::runtime_error("no faces to sew in extract");
  ...
  //     builder.Dump();
    ...
    if (output.IsNull())
      throw std::runtime_error("NULL shape in sewing");
    
    TopoDS_Shape nc = occt::getFirstNonCompound(output);
    if (nc.IsNull())
      throw std::runtime_error("NULL shape for first non compound in sewing");
    ...
    if (out.IsNull())
      throw std::runtime_error("null output shape in sewing");
    
    ShapeCheck check(out);
    if (!check.isValid())
      throw std::runtime_error("shapeCheck failed in extract feature");
